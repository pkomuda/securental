    ...
    const handleSubmit = () => {
        if (!!(validate(changePasswordRequest, errors, setErrors, schema) & validatePasswords())) {
            Swal.fire({
                ...
                input: "password",
                preConfirm: otpCode => {
                    axios.put(`/changePassword/${username}`,
    ...
    return (
        <React.Fragment>
            <Breadcrumb>
                ...
                <LinkContainer to={`/accountDetails/${username}`} exact>
                ...
                <Row className="justify-content-center">
                    <Col sm={6} className="form-container">
                        ...
                        <ButtonToolbar className="justify-content-center">
                            <Button id="back"
                                    onClick={() => props.history.push(`/accountDetails/${username}`)}>{t("navigation.back")}</Button>
                            <Button id="ok"
